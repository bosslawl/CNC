...
func CreateUser(session *Sessions.Session, cmd []string) error {
	...
	if error != nil {
		session.Channel.Write([]byte("MaxTime options must be a integer\r\n"))
		return nil
	}

	Cooldown, error := strconv.Atoi(Cooldw)
	if error != nil {
		session.Channel.Write([]byte("Cooldown options must be a integer\r\n"))
		return nil
	}

	Concurrents, error := strconv.Atoi(conns)
	if error != nil {
		session.Channel.Write([]byte("Concurrents options must be a integer\r\n"))
		return nil
	}

	DaysActive, error := strconv.Atoi(Days)
	if error != nil {
		session.Channel.Write([]byte("Days active options must be a integer\r\n"))
		return nil
	}
