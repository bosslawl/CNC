...
func VIPTrue(session *Sessions.Session, cmd []string) error {

	if len(cmd) <= 2 {
		session.Channel.Write([]byte("Invalid Syntax\r\n"))
		session.Channel.Write([]byte("Syntax: users vip=true <<Username>>\r\n"))
		session.Channel.Write([]byte("Example: users vip=true root\r\n"))
		return nil
	}
	...
	for LengthControl := 2; LengthControl < len(cmd); LengthControl++ {
		...
		if User.VIP {
			session.Channel.Write([]byte("\x1b[38;5;15m" + ParseJson.ConfigParse.App.AppName + " -> \"" + User.Username + "\" is already registered as a VIP user\r\n"))
			continue
		}

		Row := Database.EditFeild(cmd[LengthControl], "VIP", "1", false)
		if !Row {
			session.Channel.Write([]byte("\x1b[38;5;15m" + ParseJson.ConfigParse.App.AppName + " -> Failed to update users VIP status\r\n"))
			continue
		} else {
			session.Channel.Write([]byte("\x1b[38;5;15m\"\x1b[38;5;2m" + User.Username + "\x1b[38;5;15m\" is now VIP\x1b[38;5;15m\r\n"))
		}

		for _, I := range Sessions.Sessions {
			if I.User.Username == User.Username {
				I.Channel.Write([]byte("\x1b[0m\x1b7\x1b[1A\r\x1b[2K"))
				Execute.Execute_CustomTerm("user-vip-promoted", I.User, I.Channel, true, nil)
